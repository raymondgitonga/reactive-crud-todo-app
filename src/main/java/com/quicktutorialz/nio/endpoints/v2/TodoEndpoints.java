...
public class TodoEndpoints extends Endpoints {
    ...
    Action readToDo = (HttpServletRequest request, HttpServletResponse response)-> {
        Observable.just(request)
                ...
                .subscribe(output -> toJsonResponse(request, response, new ResponseDto(200, output.isPresent()
                        ? output.get() : "todo not found")));
    };
    @Api(path = "/api/v2/read", method = "POST",  produces = "application/json")
    ...
    @Api(path = "/api/v2/delete/{id}", method = "POST", consumes = "application/json")
    Action deleteToDo = (HttpServletRequest request, HttpServletResponse response)-> {
        Observable.just(request)
                .map(req -> getPathVariables(request).get("id"))
                .flatMap(id -> toDoDao.delete(id))
                .subscribe(output -> toJsonResponse(request, response, new ResponseDto(200, output ? "todo deleted" : "todo not found")));
    };
